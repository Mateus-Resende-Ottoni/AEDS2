...
// Ordenação parcial por heapsort (Chave primária: Altura)
void construir (Pokemon *pokedex, int tamanho, XData process_data){

  int n = tamanho;

  process_data->comparacoes += 1;
  while ( (n > 1) && ( (Poke_get_height(pokedex[n]) > Poke_get_height(pokedex[n/2])) ||
          ( (Poke_get_height(pokedex[n]) == Poke_get_height(pokedex[n/2])) &&
            (strcmp( Poke_get_name(pokedex[n]), Poke_get_name(pokedex[n/2]) ) > 0 ) ) ) ) {
...
int getMaiorFilho (Pokemon *pokedex, int i, int tamanho, XData process_data){

  int filho;

  process_data->comparacoes += 1;
  if ( 2*i == tamanho ||
     ( Poke_get_height(pokedex[2*i]) > Poke_get_height(pokedex[2*i+1]) ) ||
     ( (Poke_get_height(pokedex[2*i]) == Poke_get_height(pokedex[2*i+1])) &&
       (strcmp( Poke_get_name(pokedex[2*i]), Poke_get_name(pokedex[2*i+1]) ) > 0) ) ) {
    filho = 2*i;
  }
  else {
    filho = 2*i + 1;
...
void reconstruir (Pokemon *pokedex, int tamanho, XData process_data){

  int i = 1;

  
  while( i <= (tamanho/2) ){
    int filho = getMaiorFilho(pokedex, i, tamanho, process_data);

    process_data->comparacoes += 1;
    if( ( Poke_get_height(pokedex[i]) < Poke_get_height(pokedex[filho]) ) ||
     ( (Poke_get_height(pokedex[i]) == Poke_get_height(pokedex[filho])) &&
       (strcmp( Poke_get_name(pokedex[i]), Poke_get_name(pokedex[filho]) ) < 0) ) ) {
...
// Muito dos valores utilizados nessa execução foram obtidos ao longo de variadas execuções
//  buscando o resultado esperado pelo arquivo pub.out, por isso não corresponde a estrutura
//  padrão de um heapsort parcial.
void heapsort_parcial (Pokemon *pokedex, int n, int k, XData process_data) {

  //Alterar o vetor ignorando posição zero
  Pokemon temp[n+1];
  for(int i = 0; i < n; i++){
    temp[i+1] = pokedex[i];
  }
  ...
  //Contrução do heap
  for(int tamanho = 2; tamanho <= n/2; tamanho++){
    construir(temp, tamanho, process_data);
  }

  // Apesar que essa estrutura é esperada de um heapsort parcial, pelo resultado do pub.out,
  //  entende-se que não foi usada.
  //for (int i = k-1; i <= ((n/2)); i++) {
  //  process_data->comparacoes += 1;
  //  if ( ( Poke_get_height(pokedex[i]) < Poke_get_height(pokedex[1]) ) ||
  //       ( (Poke_get_height(pokedex[i]) == Poke_get_height(pokedex[1])) &&
  //         (strcmp( Poke_get_name(pokedex[i]), Poke_get_name(pokedex[1]) ) < 0) ) ) {
  //    process_data->movimentacoes += 3;
  //    swap (temp, i, 1);
  //    reconstruir(temp, n/2, process_data);
  //  }
  //}
  ...
  while(tamanho > 1){
    process_data->movimentacoes += 3;
    swap(temp, 1, tamanho--);
    reconstruir(temp, tamanho, process_data);
  }
...
int cont = 0; int x = 0;
int k = 10;
char* log_name = "matrícula_heapsort.txt";
char idPoke[5];
...
heapsort_parcial(my_pokedex, cont, k, process_data);
