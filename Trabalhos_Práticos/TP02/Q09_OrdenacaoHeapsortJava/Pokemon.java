...
class Pokemon {
    ...
    static void construir(Pokemon[] pokedex, int tamanho, int[] comparacoes, int[] movimentacoes) {
        for (int i = tamanho; i > 1 &&
            (pokedex[i].getHeight() > pokedex[i / 2].getHeight() ||
             (pokedex[i].getHeight() == pokedex[i / 2].getHeight() && !(nameCompare(pokedex[i], pokedex[i/2]))) );
            i /= 2) {
            movimentacoes[0] += 3;
            comparacoes[0]++;
            swap(pokedex, i, i / 2);
    ...
    public static void main(String[] args) {
        ...
        String nome_arquivo = "matrícula_heapsort.txt";
        boolean[] found;
        String pokemon_nome;
        ...
        comparacoes[0] = 0;
        movimentacoes[0] = 0;
        // Organização por HeapSort e tempo do processo
        inicio = pokedex[0].now();
        heapsort(pokedex, comparacoes, movimentacoes);
        fim = pokedex[0].now();
