...
// Organização pelo método de seleção com recursividade
void selecaoRecursiva (Pokemon* pokedex, int n, int x, XData process_data) {

  int menor = x;

  for (int y = (x + 1); y < n; y++){
    process_data->comparacoes++;
    if ( strcmp(Poke_get_name(pokedex[y]), (Poke_get_name(pokedex[menor])) ) < 0) {
      menor = y;
    }
  ...
  if (menor != x) {
    process_data->movimentacoes += 3;
    swap(pokedex, x, menor);
  }

  if (x < n) {
    selecaoRecursiva (pokedex, n, x+1, process_data);
  }
}

void selecaoRecursiva_call (Pokemon* pokedex, int n, XData process_data) {
  selecaoRecursiva(pokedex, n, 0, process_data);
}
...
int cont = 0; int x = 0;
char* log_name = "matrícula_selecaoRecursiva.txt";
char idPoke[5];
...
selecaoRecursiva_call(my_pokedex, cont, process_data);
