...
class Pokemon {
    ...
    static void counting(Pokemon[] pokedex, int[] comparacoes, int[] movimentacoes) {
        ...
        // via método de bolha
        // Método desativado pois o pub.out foi corrigido
        /*
         * boolean organizado = false;
         * while (!(organizado)) {
         * organizado = true;
         ...
         * 
         * comparacoes[0] += 2;
         * if ((pokedex_ordenada[i].getCapture_rate() == pokedex_ordenada[i +
         * 1].getCapture_rate()) &&
    ...
    public static void intercalar(Pokemon[] pokedex, int esquerda, int meio, int direita, int[] comparacoes,
            int[] movimentacoes) {
        int n1, n2, i, j, k;
    ...
    // Ordenação parcial por quicksort (Chave primária: Generation)
    // Apesar de não dito no enunciado, analisando o arquivo de saída pub.out
    // pode-se concluir que não se percorre o array inteiro, por isso
    // o incomum uso de apenas analisar metade do array
    public static void quicksort_parcial_call(Pokemon[] pokedex, int k, int[] comparacoes, int[] movimentacoes) {
        quicksort_parcial(pokedex, 0, (pokedex.length - 1) / 2, k, comparacoes, movimentacoes);
    }

    public static void quicksort_parcial(Pokemon[] pokedex, int esquerda, int direita, int k, int[] comparacoes,
            int[] movimentacoes) {

        // Determinar limites e pivô
        int i = esquerda, j = direita;
        Pokemon pivo = pokedex[(direita + esquerda) / 2];

        while (i <= j) {

            // Achar os não condizentes aos grupos
            comparacoes[0]++;
            while ((pokedex[i].getGeneration() < pivo.getGeneration()) ||
                    (pokedex[i].getGeneration() == pivo.getGeneration() &&
                            (nameCompare(pokedex[i], pivo)))) {
                comparacoes[0]++;
                i++;
            }
            while ((pokedex[j].getGeneration() > pivo.getGeneration()) ||
                    (pokedex[j].getGeneration() == pivo.getGeneration() &&
                            (nameCompare(pivo, pokedex[j])))) {
                comparacoes[0]++;
                j--;
            }
        ...
        // Chamadas recursivas
        if (esquerda < j) {
            quicksort_parcial(pokedex, esquerda, j, k, comparacoes, movimentacoes);
        }
        // if (i < k && i < direita) {
        if (i < direita) {
            quicksort_parcial(pokedex, i, direita, k, comparacoes, movimentacoes);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        int k = 10;
        String nome_arquivo = "matrícula_countingsort.txt";
        boolean[] found;
        String pokemon_nome;
        ...
        counting(pokedex, comparacoes, movimentacoes);
        fim = pokedex[0].now();
