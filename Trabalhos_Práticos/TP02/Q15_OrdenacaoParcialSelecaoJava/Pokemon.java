...
class Pokemon {
    ...
    public static boolean find(Pokemon[] pokedex, String busca, int[] comparacoes) {
        ...
        while (found == false && x < pokedex.length) {
            ...
            if (pokedex[x].name.length() == busca.length()) {
                found = true;
                // Comparar cada caracter individualmente
                for (int y = 0; y < busca.length(); y++) {
                    if (busca.charAt(y) != pokedex[x].name.charAt(y)) {
                        found = false;
                    }
                }
            }
    ...
    // Seleção parcial
    static void selecao_parcial(Pokemon[] pokedex, int k, int[] comparacoes, int[] movimentacoes) {
    ...
    public static void save_log(String nome_arquivo, double inicio, double fim, int[] comparacoes, int[] movimentacoes) {
    ...
    public static void main(String[] args) {
        ...
        String nome_arquivo = "matrícula_selecao.txt";
        boolean[] found;
        String pokemon_nome;
        int ordenacao_parcial = 10;
        ...
        selecao_parcial(pokedex, ordenacao_parcial, comparacoes, movimentacoes);
        fim = pokedex[0].now();
