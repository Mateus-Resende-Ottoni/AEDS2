...
/* Métodos para a struct Pokedex Lista */
...
/* Métodos para a struct Pokedex Fila Circular */

/* Definições */
// Definição do tipo Pokedex_FilaCircular
typedef struct Pokemon_FilaCircular
{
    Pokemon *fila;
    int length;
    int primeiro;
    int ultimo;
} Pokedex_FilaCircular;

typedef Pokedex_FilaCircular *Pokedex_FC;
/* */

/* Construtores */
Pokedex_FC new_Pokedex_FilaCircular(int x)
{

    Pokedex_FC nova = null;

    nova = malloc(1 * sizeof(Pokedex_FC));
    ...
    if (nova)
    {
        ...
        nova->primeiro = 0;
        nova->ultimo = 0;
        nova->fila = malloc(x+1 * sizeof(Pokemon));
    }
...
int Dex_FC_set_primeiro(Pokedex_FC my_pokedex, int x)
{
    my_pokedex->primeiro = x;
}

int Dex_FC_set_ultimo(Pokedex_FC my_pokedex, int x)
{
    my_pokedex->ultimo = x;
}

void Dex_FC_set_Poke(Pokedex_FC my_pokedex, Pokemon to_set, int x)
{
    my_pokedex->fila[x] = to_set;
}
...
int Dex_FC_get_primeiro(Pokedex_FC my_pokedex)
{
    return (my_pokedex->primeiro);
}

int Dex_FC_get_ultimo(Pokedex_FC my_pokedex)
{
    return (my_pokedex->ultimo);
}

int Dex_FC_get_length(Pokedex_FC my_pokedex)
{
    return (my_pokedex->length);
}

Pokemon Dex_FC_get_Poke(Pokedex_FC my_pokedex, int x)
{
    return (my_pokedex->fila[x]);
}
...
Pokemon Dex_FC_Remover(Pokedex_FC my_pokedex)
{
    Pokemon removido;
    int ultimo = Dex_FC_get_ultimo(my_pokedex);
    int primeiro = Dex_FC_get_primeiro(my_pokedex);
    int length = Dex_FC_get_length(my_pokedex);

    if (primeiro != ultimo)
    {

        removido = Dex_FC_get_Poke(my_pokedex, primeiro);
      //Dex_FC_set_Poke(my_pokedex, new_Poke(), primeiro);

        Dex_FC_set_primeiro(my_pokedex, ((primeiro+1) % length) );
...
void Dex_FC_Inserir(Pokedex_FC my_pokedex, Pokemon to_set)
{
    int ultimo = Dex_FC_get_ultimo(my_pokedex);
    int primeiro = Dex_FC_get_primeiro(my_pokedex);
    int length = Dex_FC_get_length(my_pokedex);

    if (primeiro == ((ultimo+1) % length) ) {
        Dex_FC_Remover(my_pokedex);
    }
    
    Dex_FC_set_Poke(my_pokedex, to_set, ultimo);
    Dex_FC_set_ultimo(my_pokedex, ((ultimo+1) % length) );
...
int Dex_FC_Median_CR(Pokedex_FC my_pokedex)
{
    int i = Dex_FC_get_primeiro(my_pokedex);
    int ultimo = Dex_FC_get_ultimo(my_pokedex);
    int length = Dex_FC_get_length(my_pokedex);
    int capture_rate = 0;
    int n = 0; int sum = 0;
    ...
    while (i != ultimo) {
        capture_rate = Poke_get_capture_rate( Dex_FC_get_Poke(my_pokedex, i) );
        sum = sum + capture_rate;
        n++;
        //printf("n: %d, cr: %d, sum: %d\n", n, capture_rate, sum);
        i = ( (i+1) % length );
    }
...
void Dex_FC_print(Pokedex_FC my_pokedex)
{
    int i = Dex_FC_get_primeiro(my_pokedex);
    int cont = 0;
    int length = Dex_FC_get_length(my_pokedex);
    ...
    while (i != ultimo) {
        printf("[%d] ", cont);
        Poke_print( Dex_FC_get_Poke(my_pokedex, i) );
        i = ( (i+1) % length );
        cont++;
    }
...
void main(void)
{
    ...
    // Definir pokedex
    Pokedex_FC my_pokedex = new_Pokedex_FilaCircular(5);
	...
	Dex_FC_Inserir(my_pokedex, to_insert);

        media = Dex_FC_Median_CR(my_pokedex);
        printf("Média: %d\n", media);
    ...
    for (y = 0; y <= cont; y++) {
        ...
        // Remoções e print dos removidos
            ...
            removido = Dex_FC_Remover(my_pokedex);
