...
/* Definições */
// Definição do tipo Pokedex_Pilha
typedef struct Pokemon_Pilha
{
    Pokemon *lista;
    int length;
    int n;
...
typedef Pokedex_Pilha *Pokedex_P;
/* */

/* Construtores */
Pokedex_P new_Pokedex_Pilha(int tamanho)
{

    Pokedex_P nova = null;

    nova = malloc(1 * sizeof(Pokedex_P));

    if (nova)
    {
        nova->length = tamanho;
        nova->n = 0;
        nova->lista = malloc(tamanho * sizeof(Pokemon));
    }
...
int Dex_P_set_n(Pokedex_P my_pokedex, int x)
{
    my_pokedex->n = x;
}

void Dex_P_set_Pokes(Pokedex_P my_pokedex, Pokemon *to_set)
{
    my_pokedex->lista = to_set;
}

void Dex_P_set_Poke(Pokedex_P my_pokedex, Pokemon to_set, int x)
{
...
int Dex_P_get_n(Pokedex_P my_pokedex)
{
    return (my_pokedex->n);
}

int Dex_P_get_length(Pokedex_P my_pokedex)
{
    return (my_pokedex->length);
}

Pokemon Dex_P_get_Poke(Pokedex_P my_pokedex, int x)
{
...
/* Método de troca */
void swap_L(Pokedex_P my_pokedex, int pos1, int pos2)
{

    Pokemon temp = Dex_P_get_Poke(my_pokedex, pos1);
    Dex_P_set_Poke(my_pokedex, Dex_P_get_Poke(my_pokedex, pos2), pos1);
    Dex_P_set_Poke(my_pokedex, temp, pos2);
}
/* */

/* Método de remoção */

Pokemon Dex_P_Remover(Pokedex_P my_pokedex)
{
    Pokemon removido;
    int n = Dex_P_get_n(my_pokedex);

    if (n > 0)
    {
        removido = Dex_P_get_Poke(my_pokedex, n - 1);

        n--;
        Dex_P_set_n(my_pokedex, n);
    }
...
void Dex_P_Inserir(Pokedex_P my_pokedex, Pokemon to_set)
{
    int n = Dex_P_get_n(my_pokedex);
    int length = Dex_P_get_length(my_pokedex);
    if (n < length)
    {
        Dex_P_set_Poke(my_pokedex, to_set, n);
...
void main(void)
{
    ...
    // Definir pokedex
    Pokedex_P my_pokedex = new_Pokedex_Pilha(cont + 25);
    ...
    while (x < cont)
    {
        ...
        Dex_P_Inserir(my_pokedex, to_insert);
    ...
    for (int y = 0; y <= cont; y++)
    {
        ...
        else if (operation[0] == 'R')
        {
            ...
            removido = Dex_P_Remover(my_pokedex);
    ...
    int length = Dex_P_get_n(my_pokedex);
    for (int y = 0; y < length; y++)
    {
        printf("[%d] ", y);
        Poke_print(Dex_P_get_Poke(my_pokedex, y));
    }
